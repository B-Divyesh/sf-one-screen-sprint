...
const videoDir = '/tmp/one-screen-sprint-verification-5-video';
await mkdir(videoDir, { recursive: true });

const results = {
  checkedAt: new Date().toISOString(),
  runtimeImplementation: '6584718359bf7ca6daff94c7c3b9ff8126e7c82b',
  documentationCommit: 'f9a178b0e96989240e69bc0be7b237856319b560',
  firstScreen: {},
  sample: {},
  movementEffects: {},
  routes: {},
...
for (const route of ['/', '/demo', '/privacy', '/terms', '/404.html']) {
  ...
  const undersized = await phonePage.locator('a[href], button, input:not([type="hidden"]), select, textarea').evaluateAll((elements) => elements.flatMap((element) => {
  ...
  assert.deepEqual(undersized, [], `${route} has undersized targets`);
  phoneChecks[route] = { undersizedTargets: 0 };
}
await phonePage.goto(base, { waitUntil: 'networkidle' });
...
for (const selector of ['.audience', '.hero-actions span', '.plain-facts', '.game-foot', '.site-footer', '.site-header nav a', 'button']) {
  const value = await phonePage.locator(selector).evaluateAll((elements) => Math.min(...elements
    .filter((element) => element.getClientRects().length > 0)
    .map((element) => Number.parseFloat(getComputedStyle(element).fontSize))));
  assert.ok(value >= 17, `${selector} is ${value}px`);
  essentialFontSizes[selector] = value;
}
await phonePage.evaluate(() => { document.documentElement.style.fontSize = '200%'; });
const reflow = await phonePage.evaluate(() => ({
...
assert.ok(reflow.scrollWidth <= reflow.clientWidth + 1);
assert.equal(reflow.actionVisible && reflow.canvasVisible, true);
results.accessibility.phone = { routes: phoneChecks, essentialFontSizes, reflow };
...
const expectedRoutes = {
  ...
  '/verification-5-missing': ['Page not found — One Screen Sprint', 'Find the race from the start', 404],
};
...
const skipLink = routePage.getByRole('link', { name: 'Skip to game and page content' });
...
assert.equal(await routePage.evaluate(() => location.hash), '#main');
await routePage.getByRole('button', { name: 'Settings' }).focus();
await routePage.keyboard.press('Enter');
const dialog = routePage.getByRole('dialog', { name: 'Game settings' });
assert.equal(await dialog.isVisible(), true);
assert.equal(await routePage.evaluate(() => document.querySelector('dialog')?.matches(':modal')), true);
...
await routePage.waitForFunction(() => document.activeElement?.tagName === 'H1');
assert.equal(await routePage.locator('h1').evaluate((element) => element === document.activeElement), true);
results.accessibility.keyboard = { skipLink: true, focusStyle, dialogModal: true, escapeRestoredFocus: true, routeFocus: true };

await routePage.goto(`${base}/privacy`, { waitUntil: 'networkidle' });
await routePage.evaluate(() => {
  localStorage.setItem('one-screen-sprint:game', JSON.stringify({ sample: false }));
  localStorage.setItem('one-screen-sprint:settings', JSON.stringify({ muted: true, effects: false, assist: true }));
});
routePage.once('dialog', (item) => item.accept());
...
const remainingRealKeys = await routePage.evaluate(() => Object.keys(localStorage).filter((key) => key.startsWith('one-screen-sprint:')));
assert.deepEqual(remainingRealKeys, []);
results.privacy.clearSavedData = { announced: true, remainingRealKeys };

await routePage.goto(base, { waitUntil: 'networkidle' });
const internalLinks = await routePage.locator('a[href]').evaluateAll((anchors) => [...new Set(anchors
  .map((anchor) => new URL(anchor.href, location.href))
  .filter((url) => url.origin === location.origin)
  .map((url) => `${url.pathname}${url.search}`))]);
const linkResults = [];
for (const href of internalLinks) {
  const response = await routePage.request.get(`${base}${href}`);
  linkResults.push({ href, status: response.status() });
  assert.equal(response.status(), 200, `${href} did not return 200`);
}
results.links = { internal: linkResults };
